<template>
  <div class="lecture-detail">
    ...
    <div class="form-group">
      ...
      <input type="text" name="title" :value="survey_question_no" class="form-input" readonly />
    </div>
    ...
    <div class="form-radio">
      ...
      <input type="radio" id="option1" name="survey_question_type"  v-model="selectedQuestionType" value="choice" /><label for="option1">선택형</label>
      <input type="radio" id="option2" name="survey_question_type" v-model="selectedQuestionType" value="written" /><label for="option2">서술형</label>
    </div>
    ...
    <div v-if="selectedQuestionType === 'choice'">
      ...
      <select v-model="selectedCategory" style="height:70px; width:200px" size="3">
        <option value="none" disabled>=== 선택 ===</option>
        <option v-for="(item) in categoryList" :key="item.question_choiced" :value="item.question_choiced">
          {{ item.category_name }}
        </option>
      </select>
...
export default {
  ...
  methods: {
    fetchCategoryList() {
      axios.get('/survey/categoryList.do')
        .then(response => {
    ...
    updateNotice() {
      ...
      axios.post('/survey/questionUpdate.do', params)
        .then(response => {
    ...
    deleteNotice() {
      if (confirm(this.localSurveyQuestionText + " 이 질문을 정말로 삭제하시겠습니까?")) {
        axios.delete('/survey/questionDelete.do', {
          params: {
    ...
    insertNotice() {
      ...
      axios.post('/survey/questionInsert.do', params)
      .then(response => {
